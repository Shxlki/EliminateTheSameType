...
public class GameManger : MonoBehaviour
{
    ...
    private float currentMove;//当前步数
    ...
    public SweetPosition[] positionSweet;
    ...
    private void Awake()
    {
        ...
        sweets = new GameSweet[x, y];

        for(int i=0;i<positionSweet.Length;i++)
        {
            if(positionSweet[i].xD>=0&&positionSweet[i].xD<x&&positionSweet[i].yD>=0&&positionSweet[i].yD<y)
            {
                CreateNewSweet(positionSweet[i].xD, positionSweet[i].yD, positionSweet[i].type);
            }
        }
        for (int i = 0; i < x; i++)
        {
        ...
        CreateNewSweet(4, 3, SweetType.BARRIER);
        Destroy(sweets[1, 1].gameObject);
        CreateNewSweet(1, 1, SweetType.BARRIER);
        Destroy(sweets[1, 1].gameObject);
        CreateNewSweet(1, 1, SweetType.BARRIER);
        Destroy(sweets[7, 1].gameObject);
        CreateNewSweet(7, 1, SweetType.BARRIER);
        Destroy(sweets[1, 6].gameObject);
        CreateNewSweet(1, 6, SweetType.BARRIER);
        Destroy(sweets[7, 6].gameObject);
        CreateNewSweet(7, 6, SweetType.BARRIER);*/
    ...
    void Update()
    {
        ...
        if (gameTime <= 0||(numMoves-movesUsed==0))
        {
            gameTime = 0;
            //显示失败面板，播放失败面板动画
    ...
    public bool Fill()
    {
        ...
        for (int i = y - 2; i >= 0; i--)
        {
            for (int j = 0; j < x; j++)
            {
                GameSweet sweet = sweets[j, i];//当前元素位置的甜品对象
                if (sweet.CanMove())//如果无法移动，则无法向下填充
                {
                    GameSweet sweetBelow = sweets[j, i + 1];
        ...
        for (int i = 0; i < x; i++)
        {
            ...
            if (sweet.Type == SweetType.EMPTY)
            {
                ...
                sweets[i, 0].ColorComponent.SetColor((SweetColor.ColorType)Random.Range(0, sweets[i, 0].ColorComponent.NumColors));
                isFill = true;
            }
        }
    ...
    private bool IsFriend(GameSweet sweet1, GameSweet sweet2)//判断甜品是否相邻
    {
    ...
    private void ExchangeSweets(GameSweet sweet1, GameSweet sweet2)
    {
        if (sweet1.CanMove() && sweet2.CanMove())
        {
            ...
            if (MatchSweets(sweet1, sweet2.X, sweet2.Y) != null || MatchSweets(sweet2, sweet1.X, sweet1.Y) != null || sweet1.Type == SweetType.RAINBOWANDY || sweet2.Type == SweetType.RAINBOWANDY)
            {
                ...
                {
                    SweetColorClear clearColor = sweet1.GetComponent<SweetColorClear>();
                    if (clearColor != null)
                    {
                ...
                {
                    SweetColorClear clearColor = sweet2.GetComponent<SweetColorClear>();
                    if (clearColor != null)
                    {
    ...
    #region
    public void PressSweet(GameSweet sweet)//鼠标按下甜品
    {
        if (gameOver)
        {
            return;
    ...
    public List<GameSweet> MatchSweets(GameSweet sweet, int newX, int newY)
    {
        if (sweet.CanColor())
        {
            SweetColor.ColorType color = sweet.ColorComponent.Color;
    ...
    private bool ClearAllMatchedSweet()
    {
        ...
        for (int y1 = 0; y1 < y; y1++)
        {
            for (int x1 = 0; x1 < x; x1++)
            {
                ...
                {
                    List<GameSweet> matchList = MatchSweets(sweets[x1, y1], x1, y1);//拿完成匹配成功的列表
                    if (matchList != null)
                    {
                        SweetType specialSweetsType = SweetType.COUNT;//决定是否产生特殊甜品

                        GameSweet randomSweet = matchList[Random.Range(0, matchList.Count)];//获取随机产生的甜品坐标位置
                        ...
                        if (specialSweetsType != SweetType.COUNT)//当前位置不是标记类型的甜品
                        {
                            ...
                            GameSweet newSweet = CreateNewSweet(specialSweetX, specialSweetY, specialSweetsType);//产生新甜品
                            ...
                            {
                                newSweet.ColorComponent.SetColor(SweetColor.ColorType.ANY);
                            }
                        }
                    }
                }
    ...
    public void ClearColor(SweetColor.ColorType color)//获取当前同类型的颜色
    {
        for (int i = 0; i < x; i++)
        {
            for (int j = 0; j < y; j++)
            {
                ...
                if (sweets[i, j].CanColor() && (sweets[i, j].ColorComponent.Color == color || color == SweetColor.ColorType.ANY))
                {
                    ClearSweet(i, j);
                }
            }
    ...
    {
        movesUsed++;
        currentMove = numMoves - movesUsed;
        Debug.Log("Move:" + currentMove);
        move.text = currentMove.ToString("0");
    }
}
